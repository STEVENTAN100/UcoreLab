...
/*
 * (3)_clock_map_swappable: According CLOCK PRA, we should link the most recent arrival page at the back of pra_list_head queue
 */
static int
    ...
    while (1) {
        ...
        if (entry == head) {
            *ptr_page = NULL;
            ...
            while(le2page(entry, pra_page_link)->visited) {
                le2page(entry, pra_page_link)->visited = 0;
                entry = list_prev(head);
                if(entry == head) entry = list_prev(head);
                curr_ptr = entry;
            }
            list_del(entry);
